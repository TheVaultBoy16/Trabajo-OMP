/* prod_escalar_mpi.c ejercicio 7 en vez de una suma, un minimo
...
main(int argc, char** argv) {
    ...
    x_par= malloc(n_local*sizeof(float));
    y_par= malloc(n_local*sizeof(float));

    MPI_Scatter(&x[0],n_local,MPI_FLOAT,&x_par[0],n_local,MPI_FLOAT,0,MPI_COMM_WORLD); // AÑADIDO

    MPI_Scatter(&y[0],n_local,MPI_FLOAT,&y_par[0],n_local,MPI_FLOAT,0,MPI_COMM_WORLD); // AÑADIDO
    suma_local = prod_escalar_serie(&x_par[0], &y_par[0], n_local);

    /*
    n_local = n/p;
    x_par=malloc(n_local*sizeof(float)); // para reserva de memoria dinamica
    MPI_Scatter(x,n_local,MPI_FLOAT,x_par,n_local,MPI_FLOAT,0,MPI_COMM_WORLD);
    min_local=minimo(x_par,n_local);
    inicio_vector_local = mi_rango * n_local;
    ...
    */
    ...
    // quitamos bucle de arriba y lo sustituimos por mpi_reduction
    MPI_Reduce(&suma_local, &suma_total, 1, MPI_FLOAT, MPI_MIN,0,MPI_COMM_WORLD);
